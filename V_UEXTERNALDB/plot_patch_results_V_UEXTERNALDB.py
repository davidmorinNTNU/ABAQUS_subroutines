...
filenames = ['example_UEXTERNALDB_data','example_VEXTERNALDB_data']
# Read csv files
results = {}
...
for key in list(results.keys()):
    plt.plot(results[key]['disp'],results[key]['force']/1000.0,label=key.split('_')[1])
...
plt.grid()
plt.tight_layout()
plt.savefig('UEXTERNALDB_VEXTERNALDB.pdf')
plt.show()
##########################################################################################
# END OF THE SCRIPT
##########################################################################################
